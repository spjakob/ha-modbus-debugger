...
async def test_late_response_recovery_mocked():
    config = {"connection_type": CONNECTION_TYPE_TCP, "host": "127.0.0.1", "port": 502, "name": "Test"}
    from unittest.mock import MagicMock, patch
    with patch("custom_components.ha_modbus_debugger.actions.scan.SyncModbusClient") as MC:
        ...
        result = await loop.run_in_executor(None, _run_scan_sync, config, 107, 107, 0, 3, 1.0, 0, "detailed", False)

        # Should find both 107 (from execute) and 101 (from late_responses)
        units = [d["unit_id"] for d in result["found_devices"]]
        assert 107 in units
        assert 101 in units
        assert any("Late Recovery" in str(d.get("value")) for d in result["found_devices"] if d["unit_id"] == 101)
