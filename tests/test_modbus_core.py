...
from custom_components.ha_modbus_debugger.modbus_core.protocol import (
    compute_crc, build_rtu_request, build_tcp_request, validate_rtu_crc, parse_response_pdu
)
from custom_components.ha_modbus_debugger.modbus_core.exceptions import ModbusExceptionResponseError
class TestModbusProtocol(unittest.TestCase):
    ...
    def test_build_rtu_request(self):
        ...
        self.assertEqual(build_rtu_request(1, 3, data), bytes.fromhex("010300000001840A"))
    def test_build_tcp_request(self):
        data = struct.pack(">HH", 0, 1)
        self.assertEqual(build_tcp_request(5, 1, 3, data), bytes.fromhex("000500000006010300000001"))
